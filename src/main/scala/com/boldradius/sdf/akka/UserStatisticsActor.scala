...
class UserStatisticsActor extends Actor with ActorLogging {
  ...
  private var topBrowsersByUser: Map[String, Int] = Map.empty
  private var topReferrersByUser: Map[String, Int] = Map.empty
  ...
  override def receive: Receive = {
    case Requests(requests) =>
      ...
      topBrowsersByUser ++= top(topCounts, allRequests, UserStatisticsActor.groupByBrowser, UserStatisticsActor.mapToUserCount)
      topReferrersByUser ++= top(topCounts, allRequests, UserStatisticsActor.groupByReferrer, UserStatisticsActor.mapToUserCount)
      requestsPerBrowser = requestsPerBrowser ++ all(sortedRequests, UserStatisticsActor.groupByBrowser, UserStatisticsActor.mapToCount)
